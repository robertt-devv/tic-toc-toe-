class Gcore {
	...
	arrayGroup(src,groups) {
		...
		while (pairs > 0) {
			array.push(_src.splice(0,groups).join('')); pairs--;
		}
		return array
	}
...
/*
let gameCore = new Gcore();

window.onload = function(){
	
	let buttons = gameCore._$('.-box');
	let player = 'X';
	
	for (let lp=0; lp<9; lp++){
		buttons[lp].addEventListener('click',()=>{
			gameCore.markHome('.-box',player,lp);
			player = (player == 'X') ? 'O' : 'X';
			console.log(gameCore.haveWinner('X'));
		})
	}
